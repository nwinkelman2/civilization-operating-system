...
import { GET_VOXTEX_NODES } from '../graphql/queries'
import { Network, Wifi, WifiOff, Activity } from 'lucide-react'

export default function VoxtexMesh() {
  const { data, loading } = useQuery(GET_VOXTEX_NODES)

  if (loading) return <div style={{ color: '#64748b' }}>Loading Voxtex mesh...</div>

  const nodes = data?.voxtexNodes || []
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
        ...
        <h1 style={{ fontSize: '1.5rem', fontWeight: 700 }}>Voxtex Neural Mesh</h1>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
        ...
        {nodes.length === 0 && (
          <div style={{ color: '#64748b', gridColumn: '1 / -1', textAlign: 'center', padding: '3rem' }}>
            No voxtex nodes deployed. Found a civilization to create a primary node.
          </div>
        )}
      </div>
    </div>
